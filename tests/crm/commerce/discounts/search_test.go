...
func TestSearchDiscounts(t *testing.T) {
	...
	body := discounts.SearchDiscountsJSONRequestBody{
		Limit: &limit,
		FilterGroups: []discounts.FilterGroups{{
			Filters: []discounts.Filter{{
				Operator:     discounts.FilterOperator("EQ"),
	...
	response, err := ct.SearchDiscountsWithResponse(context.Background(), body)
	if err != nil {
