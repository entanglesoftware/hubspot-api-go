...
func TestSearchOrders(t *testing.T) {
	...
	body := orders.SearchOrdersJSONRequestBody{
		Limit: &limit,
		FilterGroups: []orders.FilterGroups{{
			Filters: []orders.Filter{{
				Operator:     orders.FilterOperator("EQ"),
	...
	response, err := ct.SearchOrdersWithResponse(context.Background(), body)
	if err != nil {
